...
**Partner / referral links (affiliate revenue)** - the "what to do with the money you saved" prompt in the day-detail can surface referral buttons (e.g. Acorns, Stash) via the `PARTNERS` array + `partnerLinks()`. Each button links out with `rel="noopener sponsored nofollow"` and the block carries a plain **affiliate disclosure** ("Accountability may earn a referral bonus... education, not financial advice; do your own research"). Config-gated exactly like the funnel offers: hidden until real referral URLs replace the `YOUR_*` placeholders (`isConfigured`). This is how the free, no-tracking app earns without gating any feature or moving financial data off-device. The **landing page** mirrors this: a "For the spender" section (`#rewards` in `index.html`) tells the reward-calendar story with the calendar screenshot and reveals the same partner buttons + disclosure (its own `PARTNERS` array, `#partnerRow` / `#partnerFine`) once the URLs are set. Keep the app's and landing's `PARTNERS` values in sync.
